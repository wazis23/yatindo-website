{{-- MENU UTAMA SEKOLAH --}}
<section class="max-w-7xl mx-auto py-12 px-4 bg-white">

    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 fade-panel transition-all duration-700 ease-out">

        {{-- SMP --}}
        <a href="#" class="group relative overflow-hidden rounded-xl shadow-lg">
            <img src="/images/smp.jpg"
                 class="w-full h-56 object-cover transform group-hover:scale-110 transition duration-500">

            {{-- Overlay + teks (hidden default) --}}
            <div class="absolute inset-0 bg-blue-600/70 opacity-0 group-hover:opacity-100 transition duration-500 flex items-center justify-center text-white text-center px-4">
                <div>
                    <h3 class="text-xl font-bold">SMP Tinta Emas</h3>
                    <p class="text-xs mt-2">Pendidikan karakter & prestasi</p>
                </div>
            </div>
        </a>

        {{-- SMK --}}
        <a href="#" class="group relative overflow-hidden rounded-xl shadow-lg">
            <img src="/images/smk.jpg"
                 class="w-full h-56 object-cover transform group-hover:scale-110 transition duration-500">

            <div class="absolute inset-0 bg-yellow-500/70 opacity-0 group-hover:opacity-100 transition duration-500 flex items-center justify-center text-white text-center px-4">
                <div>
                    <h3 class="text-xl font-bold">SMK Tinta Emas</h3>
                    <p class="text-xs mt-2">Keahlian siap kerja & industri</p>
                </div>
            </div>
        </a>

        {{-- PPDB --}}
        <a href="#ppdb-section" class="group relative overflow-hidden rounded-xl shadow-lg scroll-smooth">
            <img src="/images/spmb.png"
                 class="w-full h-56 object-cover transform group-hover:scale-110 transition duration-500">

            <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition duration-500 flex items-center justify-center text-white text-center px-4">
                <div>
                    <h3 class="text-xl font-bold">SPMB Online</h3>
                    <p class="text-xs mt-2">Sistem Penerimaan Murid Baru</p>
                </div>
            </div>
        </a>

        {{-- E-Learning --}}
        <a href="#" class="group relative overflow-hidden rounded-xl shadow-lg">
            <img src="/images/elearning.jpg"
                 class="w-full h-56 object-cover transform group-hover:scale-110 transition duration-500">

            <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition duration-500 flex items-center justify-center text-white text-center px-4">
                <div>
                    <h3 class="text-xl font-bold">E-Learning</h3>
                    <p class="text-xs mt-2">Belajar online & materi digital</p>
                </div>
            </div>
        </a>

    </div>

</section>
