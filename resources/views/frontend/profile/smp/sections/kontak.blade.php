{{-- ===============================
    SECTION KONTAK & LOKASI
================================= --}}
<section class="max-w-7xl mx-auto py-20 px-4 bg-white">

    <div class="grid md:grid-cols-2 gap-16 items-start">

        {{-- ================= LEFT PANEL ================= --}}
        <div class="space-y-8 fade-panel transition-all duration-700 ease-out">

            <h2 class="text-3xl font-bold text-blue-900">
                Hubungi Kami
            </h2>

            {{-- TELEPON --}}
            <div class="flex items-start gap-4">
                <div class="bg-blue-100 text-blue-900 p-3 rounded-xl text-xl">
                    <i class="fas fa-phone"></i>
                </div>
                <div>
                    <p class="font-semibold">Telepon</p>
                    <a href="tel:+622182610808" class="text-gray-600 hover:text-blue-900 transition">
                        +622182610808
                    </a>
                </div>
            </div>

            {{-- EMAIL --}}
            <div class="flex items-start gap-4">
                <div class="bg-blue-100 text-blue-900 p-3 rounded-xl text-xl">
                    <i class="fas fa-envelope"></i>
                </div>
                <div>
                    <p class="font-semibold">Email</p>
                    <a href="mailto:info@smk-smptintaemas.sch.id"
                       class="text-gray-600 hover:text-blue-900 transition">
                        info@smk-smptintaemas.sch.id
                    </a>
                </div>
            </div>

            {{-- SOCIAL MEDIA --}}
            <div>
                <p class="font-semibold mb-4">Media Sosial</p>

                <div class="flex gap-5 text-5xl">
				
                    <a href="https://wa.me/6281380908008"
                       target="_blank"
                       class="text-green-600 hover:scale-110 transition">
                        <i class="fab fa-whatsapp"></i>
                    </a>
					
                    <a href="https://www.facebook.com/YatindoOfficial"
                       target="_blank"
                       class="text-blue-600 hover:scale-110 transition">
                        <i class="fab fa-facebook"></i>
                    </a>

                    <a href="https://youtube.com/channel/UC2SN592P1k4BIsME5CdpSCg"
                       target="_blank"
                       class="text-red-600 hover:scale-110 transition">
                        <i class="fab fa-youtube"></i>
                    </a>

                    <a href="https://www.instagram.com/smk_yatindo/"
                       target="_blank"
                       class="text-pink-600 hover:scale-110 transition">
                        <i class="fab fa-instagram"></i>
                    </a>

                    <a href="http://tiktok.com/@smk_yatindo"
                       target="_blank"
                       class="text-black hover:scale-110 transition">
                        <i class="fab fa-tiktok"></i>
                    </a>

                </div>
            </div>

        </div>


        {{-- ================= RIGHT PANEL ================= --}}
        <div class="fade-panel transition-all duration-700 ease-out">

            <h2 class="text-3xl font-bold text-blue-900 mb-6">
                Lokasi Sekolah
            </h2>

            <div class="rounded-3xl overflow-hidden shadow-xl">

                <iframe
                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3965.7393845431966!2d107.0237294!3d-6.297937900000001!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x2e699198f3bc4929%3A0x8f8b6c3cc53dc7cf!2sSMP-SMK%20Tinta%20Emas%20Indonesia%20(Yatindo)!5e0!3m2!1sen!2sid!4v1770870069164!5m2!1sen!2sid" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"
                    width="100%"
                    height="350"
                    style="border:0;"
                    allowfullscreen=""
                    loading="lazy">
                </iframe>

            </div>

        </div>

    </div>

</section>
